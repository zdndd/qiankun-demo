...
@Component({
  ...
  template: `
  <label class="checkbox"  *ngFor="let item of _config.dataSource;let i=index">
	    <input type="checkbox" [(ngModel)]="_checklist[i]" (ngModelChange)="selected($event,i)" name="{{item.id}}_{{i}}" value="{{item.value}}"> {{item.label}}
